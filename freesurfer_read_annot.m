function [vertices, label, colortable] = freesurfer_read_annot(filename)
...
if(fp < 0)
   disp(['Annotation file cannot be opened: ' filename]);
   return;
end
...
if(bool)
    ...
    if(numEntries > 0)
        ...
        for z = 1:numEntries
            len = fread(fp, 1, 'int');
            colortable.struct_names{z} = fread(fp, len, '*char')';
            colortable.struct_names{z} = colortable.struct_names{z}(1:end-1);
            colortable.table(z,1) = fread(fp, 1, 'int');
            colortable.table(z,2) = fread(fp, 1, 'int');
            colortable.table(z,3) = fread(fp, 1, 'int');
            colortable.table(z,4) = fread(fp, 1, 'int');
            colortable.table(z,5) = colortable.table(z,1) + colortable.table(z,2)*2^8 + colortable.table(z,3)*2^16 + colortable.table(z,4)*2^24;
		end
        ...
        version = -numEntries;
        if(version~=2)    
            error(['Error! Does not handle version ' num2str(version)]);
        %else
		...
		%keyboard;
		[~, T] = ismember(label, colortable.table(:,5));
%		keyboard;
        %disp(['colortable with ' num2str(colortable.numEntries) ' entries read (originally ' colortable.orig_tab ')']);
    end    
else
